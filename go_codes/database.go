...
func AddRegistrationAccept(data *model.RegistrationAccept) {
	delayValue := 50
	jitterValue := 0.5
	packetDeliveryRatioValue := 0.95

	// Set the delay field in the RegistrationAccept struct
	data.Delay = delayValue
	data.Jitter = jitterValue
	data.PacketDeliveryRatio = packetDeliveryRatioValue
